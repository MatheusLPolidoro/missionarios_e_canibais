                ...
                if pas in plataformas:
                    ...
                    if plataformas[lado + pas] > 0:
                        ...
                        while plataformas[lado + pas] < num or num < 0 or num > 2 or num + plataformas['C'] + plataformas['M'] > 2:
                            mc.Cabecalho(lado, plataformas)
                            print('A quantidade "' + str(num) + '" é invalida!! ')
                            num = mc.EntradaNum('Quantidade de' + StrPas[pas + '2'] + 'que você quer COLOCAR na canoa:', lado, plataformas) 
                        plataformas[pas] = plataformas[pas] + num
                        plataformas[lado + pas] = plataformas[lado + pas] - num
                        if plataformas[lado + 'C'] + plataformas[lado + 'M'] > 0 and plataformas['C'] + plataformas['M'] < 2 and plataformas[pas] != 0:
                            msg = ' -Ainda tem um espaço na canoa. Quer COLOCAR um' + StrPas[TrvPas[pas]] + '? [S/N]:'
                            msg = mc.EntradaTexto(msg, ['S', 'N'], lado, plataformas)
                            if msg == 'S':
...
def DescePassageiro (lado, plataformas):
    ...
    if plataformas['eC'] + plataformas['eM'] != 6:
        mc.Cabecalho(lado, plataformas)
        pas = mc.EntradaTexto(" -'C' para DEIXAR (Canibais), 'M' (Missionários), 'T' (todos) ou 'N' (Nenhum): ", ['C', 'M', 'T', 'N'],lado, plataformas)
        if pas != 'N' and pas != 'T' and plataformas['C'] + plataformas ['M'] != 0:
            while plataformas[pas] == 0:
                mc.Cabecalho(lado, plataformas)
                print('Sem' + StrPas[pas] + ' na canoa!! Você deve selecionar novamente.')
                pas = mc.EntradaTexto(" -'C' para deixar (Canibais), 'M' (Missionários), 'T' (todos) ou 'N' (Nenhum): ", ['C', 'M', 'T', 'N'],lado, plataformas)
                if not pas in plataformas:
                    break
            if pas != 'N' and pas != 'T':
                if plataformas[pas] != 1:
                    num = mc.EntradaNum('Quantos' + StrPas[pas + '2'] + 'vão DESCER do lado' + StrLado[lado] + ': ', lado, plataformas)
                    while num < 0 or num > plataformas[pas]:
                        mc.Cabecalho(lado, plataformas)
                        print('Quantidade invalida!! Você deve selecionar novamente.')
                        num = mc.EntradaNum('Quantos' + StrPas[pas + '2'] + 'vão DESCER do lado' + StrLado[lado] + ': ', lado, plataformas)
                else:
                    num = 1
                plataformas[pas] = plataformas[pas] - num
